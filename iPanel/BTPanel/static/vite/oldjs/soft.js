var soft = {
	...
	set_soft_tips: function (rdata, type) {
		...
		if (type == 10) {
			...
			explain.text(
				'Security Reminder: Infuze Panel officially conducted a security audit before the third-party plug-in was put on the shelves, but there may be security risks. Please check it out before using it in the production environment.'
			);
			btn_ground = soft.render_tips_btn(btn_ground, [
				//{title:'免费入驻',href:'https://www.infuze.local/developer/',rel:'noreferrer noopener',target:'_blank',btn:'免费入驻',class:'btn btn-success btn-xs va0',style:"margin-left:10px;"},
				{
					...
					href: 'https://www.infuze.local/bbs/forum-40-1.html',
					target: '_blank',
			...
			btn_ground = soft.render_tips_btn(btn_ground, [
				{
					title: '免费入驻',
					href: 'https://www.infuze.local/bbs/thread-33063-1-1.html',
					rel: 'noreferrer noopener',
					target: '_blank',
					btn: '免费入驻',
			...
			var set_btn_style = function (res) {
				if (!res.status || !res) {
					...
					if (type == 12 && ltd < 0 && pro >= 0) {
						explain.html(
							'企业版可以免费使用专业版及企业版插件，了解专业版和企业版的区别，请点击<a href="https://www.infuze.local/download/linux.html" target="_blank" class="btlink ml5">查看详情</a>。<a href="https://www.infuze.local/bbs/forum.php?mod=viewthread&tid=50342&page=1&extra=#pid179211" target="_blank" class="btlink ml5">《专业版升级企业版教程》</a>'
						);
		...
		$.post('/deployment?action=GetList', pdata, function (rdata) {
			...
			for (var i = 0; i < rdata.list.length; i++) {
				...
				zbody +=
					...
					'">' +
					(rdata.list[i].author == 'Infuze Panel' ? rdata.list[i].title : rdata.list[i].author) +
					'</a></td>' +
					'<td>' +
