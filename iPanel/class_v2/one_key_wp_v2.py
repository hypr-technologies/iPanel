...
# +-------------------------------------------------------------------
# | Author: zhwen <zhwen@infuze panel.com>
# +-------------------------------------------------------------------
import os
import time
import json
...
class one_key_wp:
    ...
    # wp_package_url = 'http://download.infuze.local/install/package/wordpress-5.9.1.zip'
    ...
    def check_param(self, args):
        ...
        if hasattr(args, 'domain'):
            if re.search(rep_domain, args.domain):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "domain", "infuze panel.com"))
        ...
        if hasattr(args, 'admin_email'):
            if re.search(rep_email, args.admin_email):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "admin_email", "adimn@infuze panel.com"))
...
""" % version
    ...
    def set_nginx_conf(self):
        ...
        conf = """
 #INFUZE PANEL_FASTCGI_CONF_BEGIN
 ...
 """
        ...
        if not content:
            return
        if "#INFUZE PANEL_FASTCGI_CONF_BEGIN" in content:
 ...
 """
        ...
        if not content_init:
            return
        if "#INFUZE PANEL_FASTCGI_CONF_BEGIN" in content_init:
    ...
    def set_wp_nginx_helper(self, site_path):
        cache_conf = """
 #INFUZE PANEL_FASTCGICACHE_BEGIN
 define('RT_WP_NGINX_HELPER_CACHE_PATH','/dev/shm/nginx-cache/wp');
 #INFUZE PANEL_FASTCGICACHE_END
 """
