...
class one_key_wp:
    ...
    # wp_package_url = 'http://download.hypr.local/install/package/wordpress-5.9.1.zip'
    ...
    def check_param(self, args):
        ...
        if hasattr(args, 'domain'):
            if re.search(rep_domain, args.domain):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "domain", "iPanel.com"))
        ...
        if hasattr(args, 'admin_email'):
            if re.search(rep_email, args.admin_email):
                ...
                return public.return_message(-1, 0, public.lang("Please check if the [{}] format is correct For example: {}", "admin_email", "adimn@iPanel.com"))
...
""" % version
 ...
 """
        ...
        if not content:
            return
        if "#iPanel_FASTCGI_CONF_BEGIN" in content:
 ...
 """
        ...
        if not content_init:
            return
        if "#iPanel_FASTCGI_CONF_BEGIN" in content_init:
